...
class TypeToVideoApp:
    ...
    def _ollama_worker(self, prompt: str) -> None:
        ...
        try:
            ...
            err = f"Could not reach local Ollama.\n\n{exc}"
            self.root.after(0, lambda err=err: messagebox.showerror("Ollama error", err))
            return
    ...
    def run_in_thread(self, fn, status_text: str) -> None:
        ...
        def runner():
            try:
                ...
                err = str(exc)
                self.root.after(0, lambda err=err: messagebox.showerror("Operation failed", err))
            finally:
